...
(describe "gamec"
  ...
  (context "next-occupant"
    (it "returns first occupant if no current occupant"
      (should= @tf2/heavy (sut/next-occupant [@tf2/heavy @tf2/medic @tf2/scout] nil)))

    (it "returns second occupant if on different team"
      (should= @tf2/medic (sut/next-occupant [@tf2/heavy @tf2/medic @tf2/scout] @tf2/heavy)))

    (it "returns next occupant that is on different team"
      (should= @tf2/scout (sut/next-occupant [@tf2/heavy @tf2/medic @tf2/scout] @tf2/medic)))

    (it "cycles through occupants"
      (should= @tf2/medic (sut/next-occupant [@tf2/heavy @tf2/medic @tf2/scout] @tf2/scout)))))
