(ns catchphrase.http
  (:require [catchphrase.config :as config]
            [catchphrase.layouts :as layouts]
...
(defroutes web-handler
  (refreshable 'catchphrase.routes/handler)
...
(defn app-handler []
  (if config/development?
    (let [wrap-verbose    (util/resolve-var 'c3kit.apron.verbose/wrap-verbose)
      ...
      (-> (refresh-handler 'catchphrase.http/web-handler)
          wrap-verbose))
    (util/resolve-var 'catchphrase.http/web-handler)))
...
;;  1) When the app starts, some namespaces are loaded, like this one.  But the refresh code (catchphrase.refresh)
;;      doesn't know which. As far as it knows, nothing has been loaded.  So on the first request, all the namespaces
;;      are reloaded.
;;  2) Some namespaces will/should never get reloaded. See catchphrase.refresh/excludes
...
;;        - catchphrase.routes/handler - the essence of catchphrase.com
...
(defonce root-handler
  (-> (app-handler)
      ...
      (wrap-jwt {:cookie-name "catchphrase-token" :secret (:jwt-secret config/env) :lifespan (when config/development? (time/hours 336))})
      wrap-keyword-params
